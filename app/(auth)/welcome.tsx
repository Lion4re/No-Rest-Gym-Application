  ...
  return (
    <SafeAreaView className="flex h-full items-center justify-between bg-black">
      ...
      <Swiper
        ...
        loop={false}
        dot={
          <View className="w-[32px] h-[4px] mx-1 bg-[#4A4A4A] rounded-full" />
        }
        activeDot={
          <View className="w-[32px] h-[4px] mx-1 bg-yellow-400 rounded-full" />
        }
      ...
      >
        {onboarding.map((item) => (
          <View key={item.id} className="flex items-center justify-center p-5">
            ...
            <Text className="text-md font-JakartaSemiBold text-center text-[#BBBBBB] mx-10 mt-3">
              {item.description}
            </Text>
          </View>
        ))}
